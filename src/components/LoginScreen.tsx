...
export default function LoginScreen({ onLogin, onClose }: LoginScreenProps) {
      ...
      if (isRegistering) {
        ...
        if (data.success) {
          ...
          if (loginData.success) {
            // Save local info or trigger auth context update
            try {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col justify-end">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <img 
          ...
          className="w-full h-[60%] object-cover opacity-40"
        />
      </div>
      ...
      <div className="relative z-20 flex flex-col items-center px-6 pb-8 w-full">
        ...
        {!isEmailLogin && !isRecovering && (
           <>
             ...
             <p className="text-[17px] text-gray-200 mb-14 font-medium tracking-wide">{t("welcome")}</p>
           </>
        )}

        {/* Buttons / Recovery */}
        {!isRecovering && !isEmailLogin && (
        ...
        {isEmailLogin && !isRecovering && (
           <div className="w-full bg-[#1c1c1e]/80 backdrop-blur-md p-6 rounded-2xl mb-8 border border-neutral-800">
